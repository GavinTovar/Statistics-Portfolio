...
Table.Making.LMM <- function(model){
  ...
  random_effects <- data.frame(
    ...
    Std_Dev = c(
      sqrt(model_summary$varcor$Z1[1]),  # Intercept random effect for Z1
      sqrt(model_summary$varcor$Z1[2]),  # X1 random effect for Z1
      sqrt(model_summary$varcor$Z1[3]),  # X2 random effect for Z1
      sqrt(model_summary$varcor$Z2[1]),  # Intercept random effect for Z2
      sqrt(model_summary$varcor$Z2[2]),  # X1 random effect for Z2
      sqrt(model_summary$varcor$Z2[3])   # X2 random effect for Z2
    )
  )
...
### Exploration 1
# How well does a Poisson GLMM deal with overdispersion?
...
# Same data generation but now with overdispersion via Poisson hierachy
set.seed(10262025)
sim.data0D1 <- Mixed.Model.DataGen(
  FixEff = 3,
  RanEff = 2,
  Levels = c(5, 4),
  N = 240,
...
# Same data generation but now with overdispersion via Gamma hierachy
set.seed(10262025)
sim.data0D2 <- Mixed.Model.DataGen(
  FixEff = 3,
  RanEff = 2,
  Levels = c(5, 4),
  N = 240,
...
# Same data generation but now with overdispersion via Normal hierachy
set.seed(10262025)
sim.data0D3 <- Mixed.Model.DataGen(
  FixEff = 3,
  RanEff = 2,
  Levels = c(5, 4),
  N = 240,
...
PoisMod0D1 <- glmer(Y ~ X1 + X2 + (X1 + X2 | Z1) + (X1 + X2 | Z2), 
                  data = sim.data0D1, 
                  family = poisson(link = "log"))
PoisMod0D2 <- glmer(Y ~ X1 + X2 + (X1 + X2 | Z1) + (X1 + X2 | Z2), 
                    data = sim.data0D2, 
                    family = poisson(link = "log"))
PoisMod0D3 <- glmer(Y ~ X1 + X2 + (X1 + X2 | Z1) + (X1 + X2 | Z2), 
                    data = sim.data0D3, 
                    family = poisson(link = "log"))

# Calculating overdispersion
OD.Fun(model = PoisMod0, data = sim.data0)
OD.Fun(model = PoisMod0D1, data = sim.data0D1)
OD.Fun(model = PoisMod0D2, data = sim.data0D2)
OD.Fun(model = PoisMod0D3, data = sim.data0D3)
...
# Function to extract fixed and random coefficients with their standard errors

Extract.Coeffs(PoisMod0)
Extract.Coeffs(PoisMod0D1)
Extract.Coeffs(PoisMod0D2)
Extract.Coeffs(PoisMod0D3)

### Models are too close to data generation mechanism introducing some abs Gaussian noise
err <- 3
set.seed(10262025)
sim.datazN0 <- Mixed.Model.DataGen(
  FixEff = 3,
  RanEff = 2,
  Levels = c(5, 4),
  N = 240,
  ...
  Link.function = "log",
  overdispersion = FALSE,
  Noise = rnorm(240, sd = err)
)
sim.dataN0

set.seed(10262025)
sim.dataN0D1 <- Mixed.Model.DataGen(
  FixEff = 3,
  RanEff = 2,
  Levels = c(5, 4),
  N = 240,
...
set.seed(10262025)
sim.dataN0D2 <- Mixed.Model.DataGen(
  FixEff = 3,
  RanEff = 2,
  Levels = c(5, 4),
  N = 240,
...
set.seed(10262025)
sim.dataN0D3 <- Mixed.Model.DataGen(
  FixEff = 3,
  RanEff = 2,
  Levels = c(5, 4),
  N = 240,
...
# Comparing Fits
PoisModN0 <- glmer(Y ~ X1 + X2 + (X1 + X2 | Z1) + (X1 + X2 | Z2), 
                  data = sim.dataN0, 
                  family = poisson(link = "log"))
PoisModN0D1 <- glmer(Y ~ X1 + X2 + (X1 + X2 | Z1) + (X1 + X2 | Z2), 
                    data = sim.dataN0D1, 
                    family = poisson(link = "log"))
PoisModN0D2 <- glmer(Y ~ X1 + X2 + (X1 + X2 | Z1) + (X1 + X2 | Z2), 
                    data = sim.dataN0D2, 
                    family = poisson(link = "log"))
PoisModN0D3 <- glmer(Y ~ X1 + X2 + (X1 + X2 | Z1) + (X1 + X2 | Z2), 
                    data = sim.dataN0D3, 
                    family = poisson(link = "log"))
...
# Throw some random noise on the response?
# 
